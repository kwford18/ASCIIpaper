...
CityScene::CityScene(int width, int height, int carCount, int starCount, const std::string& weather,
                     bool systemSync)
    : m_width(width), m_height(height), m_timeAccumulator(0.0f), m_baseCarCount(carCount),
      m_starCount(starCount), m_weatherStr(weather), m_systemSync(systemSync), m_trainTimer(0.0f),
      m_trainX(-50.0f), m_trainActive(false), m_trainSpeed(45.0f), m_trainCars(3),
...
void CityScene::InitializeWorld() {
    ...
    while (currentX < m_width - 5) {
        ...
        windows.reserve(static_cast<size_t>(bWidth) * static_cast<size_t>(bHeight));
        for (int i = 0; i < bWidth * bHeight; ++i) {
            windows.push_back(windowLitDist(m_rng) == 0);
        }

        m_buildings.push_back({currentX, bWidth, bHeight, static_cast<uint8_t>(colorDist(m_rng)),
                               static_cast<uint8_t>(colorDist(m_rng)),
                               static_cast<uint8_t>(colorDist(m_rng) + 20), windows});
    ...
    float spacing = static_cast<float>(m_width) / static_cast<float>(m_baseCarCount);
    ...
    for (int i = 0; i < m_baseCarCount; ++i) {
        ...
        m_cars.push_back({
            static_cast<float>(i) * spacing, // Spread them out
            yPos, speedDist(m_rng), goesRight ? Engine::Direction::Right : Engine::Direction::Left,
            static_cast<uint8_t>(carColorDist(m_rng)), static_cast<uint8_t>(carColorDist(m_rng)),
    ...
    for (int i = 0; i < m_starCount; ++i) {
        m_stars.push_back(
            {starXDist(m_rng), starYDist(m_rng), timeDist(m_rng), thresholdDist(m_rng), false});
    }
    ...
    if (m_weatherStr == "rain")
        wType = Engine::WeatherType::Rain;
    else if (m_weatherStr == "snow")
        wType = Engine::WeatherType::Snow;
    else if (m_weatherStr == "storm")
        wType = Engine::WeatherType::Storm;
...
void CityScene::UpdateTraffic(float deltaTime, float cpuMultiplier) {
    ...
    for (auto it = m_cars.begin(); it != m_cars.end();) {
        ...
        if (it->isLeaving) {
            ...
            if (it->x < -15.0f || it->x > static_cast<float>(m_width) + 15.0f) {
                it = m_cars.erase(it);
                continue;
            }
        } else {
            // Wrap around logic for permanent residents (Give it buffer space so we don't cause
            // instant collisions)
            if (it->direction == Engine::Direction::Right &&
                it->x > static_cast<float>(m_width) + 4) {
                it->x = -10.0f;
...
void CityScene::UpdateTrain(float deltaTime) {
    // Update Train
    if (!m_trainActive) {
        ...
        if (m_trainX > static_cast<float>(m_width) + 20)
            m_trainActive = false;
    }
}
...
void CityScene::UpdateUfo(float deltaTime) {
    ...
    if (m_ufo.state == UfoState::Waiting) {
        ...
        if (m_ufo.x >= static_cast<float>(m_width) / 2.0f) {
        ...
        if (m_ufo.x > static_cast<float>(m_width) + 10.0f) {
            m_ufo.state = UfoState::Waiting;
            m_ufo.timer = 0.0f;
...
void CityScene::DrawForeground(Engine::CharacterGrid& grid) {
    ...
    for (int x = 0; x < m_width; ++x) {
        ...
        if (x % 3 == 0)
            grid.SetCell(x, m_height - 5, '|', 150, 150, 150); // Steel rails
    }
    ...
    if (m_trainActive) {
        ...
        std::string trainArt = "";
        for (int i = 0; i < m_trainCars; ++i)
            trainArt += "[_]-"; // Cargo cars
...
void CityScene::DrawHUD(Engine::CharacterGrid& grid) {
    ...
    if (m_systemSync) {
        ...
        for (size_t i = 0; i < hudStr.length(); ++i) {
            grid.SetCell(2 + static_cast<int>(i), 1, hudStr[i], 0, 255,
                         255); // Glowing Cyan text in top left
        }
    }
}
