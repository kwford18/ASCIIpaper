...
void AquariumScene::InitializeWorld(int fishCount, int bubbleCount, int jellyCount,
    ...
    for (int i = 0; i < fishCount; ++i) {
        ...
        Engine::Direction dir =
            dirDist(m_rng) == 0 ? Engine::Direction::Left : Engine::Direction::Right;
        ...
        m_fishes.push_back({
            ...
            static_cast<uint8_t>(colorDist(m_rng)), static_cast<uint8_t>(colorDist(m_rng)),
    ...
    for (int i = 0; i < 6; ++i) {
        int x = swXDist(m_rng);
        while (xOccupied[x])
            x = swXDist(m_rng); // Find an empty spot
        xOccupied[x] = true;
    ...
    for (int i = 0; i < 12; ++i) {
        ...
        if (xOccupied[x])
            continue; // Skip if we couldn't find a spot
        xOccupied[x] = true;

        char sym = '&';
        if (coralTypeDist(m_rng) == 1)
            sym = '%';
        else if (coralTypeDist(m_rng) == 2)
            sym = 'Y';

        m_corals.push_back({x, m_height - 5, sym,
    ...
    // Spawn crab
    m_crab = {
        static_cast<float>(m_width) / 2.0f,  // Start in the middle
        static_cast<float>(m_height) - 4.0f, // Rest on the very bottom row
        8.0f,                                // Scurry speed
    ...
    m_whale = {static_cast<float>(m_width) + 80.0f, static_cast<float>(m_height) / 4.0f, -12.0f,
               false, 0.0f};
}
...
void AquariumScene::UpdateSystemMonitor(float deltaTime, float& cpuMultiplier,
                                        float& ramMultiplier) {
    /*
    ...
    if (m_systemSync) {
        ...
        if (activeFish < targetFishCount) {
            ...
            float startX =
                (dir == Engine::Direction::Right) ? -5.0f : static_cast<float>(m_width + 5);

            m_fishes.push_back({startX, yDist(m_rng), vx, 0.0f, dir, phaseDist(m_rng),
                                timerDist(m_rng), static_cast<uint8_t>(colorDist(m_rng)),
                                static_cast<uint8_t>(colorDist(m_rng)),
                                static_cast<uint8_t>(colorDist(m_rng)), false});
...
void AquariumScene::UpdateFishes(float deltaTime, float cpuMultiplier) {
    ...
    for (auto it = m_fishes.begin(); it != m_fishes.end();) {
        ...
        if (it->isLeaving) {
            if (it->x < -10.0f || it->x > static_cast<float>(m_width) + 10.0f) {
            ...
            } else if (it->x >= static_cast<float>(m_width) - 4.0f && it->vx > 0.0f) {
                it->x = static_cast<float>(m_width) - 4.0f;
                it->vx = -std::abs(it->vx);
            ...
            if (it->y <= 1.0f && it->vy < 0.0f) {
                it->y = 1.0f;
                it->vy = std::abs(it->vy);
            } else if (it->y >= static_cast<float>(m_height) - 6.0f && it->vy > 0.0f) {
                it->y = static_cast<float>(m_height) - 6.0f;
                it->vy = -std::abs(it->vy);
            }
        }
        ++it;
...
void AquariumScene::UpdateShrimps(float deltaTime, float cpuMultiplier) {
    ...
    for (auto& shrimp : m_shrimps) {
        ...
        if (shrimp.x <= 1.0f || shrimp.x >= static_cast<float>(m_width) - 2.0f)
            shrimp.vx = -shrimp.vx;
        if (shrimp.y <= 1.0f || shrimp.y >= static_cast<float>(m_height) - 4.0f)
            shrimp.vy = -shrimp.vy;
    }
}
...
void AquariumScene::UpdateBubbles(float deltaTime) {
    ...
    for (auto& bubble : m_bubbles) {
        ...
        if (bubble.y <= 1.0f) {
            bubble.y = static_cast<float>(m_height) - 2.0f;
...
void AquariumScene::UpdateJellyfishes(float deltaTime, float cpuMultiplier) {
    ...
    for (auto& jelly : m_jellyfishes) {
        ...
        jelly.y += jelly.speed * pulseMultiplier * cpuMultiplier *
                   static_cast<float>(jelly.verticalDir) * deltaTime;
        ...
        else if (jelly.y >= static_cast<float>(m_height) - 4.0f) {
            jelly.y = static_cast<float>(m_height) - 4.0f;
        ...
        if (jelly.x <= 1.0f)
            jelly.x = 1.0f;
        if (jelly.x >= static_cast<float>(m_width) - 3.0f)
            jelly.x = static_cast<float>(m_width) - 3.0f;
    }
}
...
void AquariumScene::UpdateCrab(float deltaTime, float cpuMultiplier) {
    ...
    if (m_crab.isMoving) {
        ...
        if (m_crab.x > static_cast<float>(m_width) - 4)
            m_crab.direction = Engine::Direction::Left;
        else if (m_crab.x < 2)
            m_crab.direction = Engine::Direction::Right;
...
void AquariumScene::DrawBackground(Engine::CharacterGrid& grid) {
    ...
    if (m_whale.active) {
        ...
        for (size_t r = 0; r < whaleArt.size(); ++r) {
            for (size_t c = 0; c < whaleArt[r].length(); ++c) {
                if (whaleArt[r][c] != ' ' && (wx + static_cast<int>(c)) >= 0 &&
                    (wx + static_cast<int>(c)) < m_width) {
                    // Dark blue/grey to look distant and massive
                    grid.SetCell(wx + static_cast<int>(c), wy + static_cast<int>(r), whaleArt[r][c],
                                 40, 60, 100);
                }
            }
        }
    ...
    for (const auto& weed : m_seaweeds) {
        ...
        for (int i = 0; i < weed.height; ++i) {
            ...
            float sway =
                std::sin(m_timeAccumulator * 2.0f + weed.swayOffset + static_cast<float>(i) * 0.5f);
...
void AquariumScene::DrawEntities(Engine::CharacterGrid& grid) {
    ...
    for (const auto& fish : m_fishes) {
        ...
        int iy = static_cast<int>(fish.y +
                                  std::sin(m_timeAccumulator * 4.0f + fish.wobbleOffset) * 0.3f);
    ...
    for (const auto& shrimp : m_shrimps) {
        ...
        int iy = static_cast<int>(shrimp.y +
                                  std::sin(m_timeAccumulator * 6.0f + shrimp.wobbleOffset) * 0.5f);
...
void AquariumScene::DrawHUD(Engine::CharacterGrid& grid) {
    ...
    if (m_systemSync) {
        std::string hudStr =
            "SYSTEM SYNC | CPU: " + std::to_string(static_cast<int>(m_sysMonitor.GetCpuUsage())) +
